using CommandLine;
using LemonTree.Pipeline.Tools.TraceReport.Checks;
using LemonTree.Pipeline.Tools.TraceReport.CommandLineOptions;
using System;
using System.IO;
using System.Text;

namespace LemonTree.Pipeline.Tools.TraceReport
{
    internal class Program
    {
        static int Main(string[] args)
        {
            ...
            return parser.ParseArguments<TraceReportOptions >(args)
                .MapResult(
                    (TraceReportOptions  opts) => RunModelCheck(opts),
                    _ => (int)Exitcode.ErrorCmdParameter);
        }

        private static int RunModelCheck(TraceReportOptions  opts)
        {
            try
            {
